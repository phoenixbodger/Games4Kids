...
document.addEventListener('DOMContentLoaded', function() {
    ...
    if (clockDisplay) { // Check if we are on the clock game page
        drawClockNumbersAndTicks(); // ✅ Add this line to render the clock face
        ...
        // --- MODIFIED: drawClockNumbersAndTicks for precise placement ---
        function drawClockNumbersAndTicks() {
  const clock = document.getElementById('clock-display');
   if (!clock) return; // ✅ Avoid crashing if element is missing
  ...
  const tickRadius = radius - borderWidth / 2 - 2; // subtract half border plus buffer
  const numberRadius = radius - 30; // slightly inset numbers for clarity
    ...
    clock.appendChild(num);
        ...
        for (let i = 0; i < 60; i++) {
            ...
            if (i % 5 === 0) { // Hour tick
                ...
                // --- MODIFIED: Adjusted multiplier for hour ticks ---
                const x = radius + (radius * 0.86) * Math.cos(angleInRadians); // Moved inwards slightly
                const y = radius + (radius * 0.86) * Math.sin(angleInRadians);

                tick.style.left = `${x}px`;
                tick.style.top = `${y}px`;
                tick.style.transform = `translate(-50%, -50%) rotate(${angleInDegrees}deg)`;
            } else { // Minute tick
                tick.classList.add('minute-tick');
                // --- MODIFIED: Adjusted multiplier for minute ticks ---
                const x = radius + (radius * 0.90) * Math.cos(angleInRadians); // Moved inwards slightly
                const y = radius + (radius * 0.90) * Math.sin(angleInRadians);
        ...
        // Functions for draggable minute hand (set-time mode)
